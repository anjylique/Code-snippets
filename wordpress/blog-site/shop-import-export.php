...
class Shop
{
    ...
    private function check()
    {
        ...
        // /var/www/tokyo.machishiru.jp/manager/wp-content/plugins/shop-import-export - 本番環境のため
    ...
    private function insert_update($path)
    {
        ...
        foreach ($files as $key => $file) {
                ...
                while (($sql = fgets($handle, 1000)) !== false) {
                    ...
                    if ($type === 'update' && $metakey !== 'post_title' && $metakey !== '') {
                        ...
                        if ($check < 1) {
                            ...
                            if ($this->site === 'miyagi.machishiru.jp' || $this->site === 'miyagiwp') {
            ...
            // unlink sql file which is the /var/www/tokyo.machishiru.jp/manager/wp-content/plugins/shop-import-export/tmp/query_{$x}.sql
            unlink($path.'/'.$file);
            $this->log('"INFO: '.$path.'/'.$file.'のファイルを削除しました。"', __LINE__, '');

            // unlink flag sql file which is the /var/www/tokyo.machishiru.jp/manager/wp-content/plugins/shop-import-export/flag/query_{$x}.sql
            unlink($flag_file);
